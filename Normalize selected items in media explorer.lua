ultrashall_api_path = reaper.GetResourcePath().."/UserPlugins/ultraschall_api.lua"
if reaper.file_exists(ultrashall_api_path) == true then
	dofile(ultrashall_api_path)
else
	retval, retvals_csv = reaper.GetUserInputs("Missing Ultrashall api", 1, "Forum Thread: ", "https://forum.cockos.com/showthread.php?t=214539")
	goto exit
end
...
function loop()
	...
	if timer < 2 then
		...
		Ostertoaster_pseudo_destructive_normalize()
...
function Ostertoaster_pseudo_destructive_normalize()
	original_path = ""
	applied_path = ""
